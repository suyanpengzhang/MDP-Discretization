p0 = importdata('covid-data/COVIDtransitions0_Greedy_200.mat');
p1 = importdata('covid-data/COVIDtransitions1_Greedy_200.mat');
...
policiesvalue=zeros(60,1);
for k = 1:60
    idx11 = find_index(0.9999,0.0001,Gs,Gi);
    total=0;
    for t1 =1:k-1
        if t1 == 1
            total = total + R(idx11,1);
            belif = p0(idx11,:);
        else
            total = total + belif*R(:,1);
            belif = belif*p0;
        end
    end
    for t2 = k:min(k+10,60)
        if t2 == 1
            total = total + R(idx11,1)-costr;
            belif = p1(idx11,:);
        else
            total = total + belif*R(:,1)-costr;
            belif = belif*p1;
        end
    end
    if t2<60
